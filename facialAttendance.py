...
def add_face_from_image():
    while True:
        ...
        if len(image_face_encodings) > 0:
            ...
            name = get_user_input("Enter the name for this face:")
            student_id = get_user_input("Enter the student ID:")
            ...
            print("Face not detected, please select a different image")

        # Ask if the user wants to add another face
        if get_user_input("Do you want to add a new face ?(yes/no)") != 'yes':
            break
...
fileName = get_user_input("Enter the date of the attendance:")

# Check if the file exists, create it if not
excel_file = fileName + ' attendance.xlsx'
if not os.path.exists(excel_file):
    df = pd.DataFrame(columns=["Name", "Student ID", "Time"])
...
while True:
    ...
    for face_encoding, face_location in zip(current_face_encodings, face_locations):
        ...
        box_color = (0, 255, 0) if name != "Unrecognizable face" else (0, 0, 255)

        # Draw a rectangle around the face and display the name and student ID (if known)
        top, right, bottom, left = face_location
        cv2.rectangle(frame, (left, top), (right, bottom), box_color, 2)
        display_text = name if name == "Unrecognizable face" else f"{name} ({student_id})"
        ...
        if name != "Unrecognizable face" and (name, student_id) not in attendance_logged_faces:
            # Append to Excel file
            current_time = datetime.now().strftime('%d-%m-%y %H:%M:%S')
            new_entry = pd.DataFrame([[name, student_id, current_time]], columns=["Name", "Student ID", "Time"])
